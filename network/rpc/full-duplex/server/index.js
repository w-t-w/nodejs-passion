...
const server = net.createServer(socket => {
    ...
    socket.on('data', buffer_data => {
        let buffer = old_buffer && old_buffer.length ? Buffer.concat([old_buffer, buffer_data]) : buffer_data;
        while ((buffer && buffer.length) && (package_length = isReceiveComplete(buffer))) {
            ...
            const buffer_result = encode(lesson, seq);
